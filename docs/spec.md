...
│   ├── fonts/                       // Pixel fonts + flat lookup
│   │   ├── include/fonts.h          // Public API: fonts_glyph, metrics, pixel_on
│   │   ├── priv_include/lvgl.h      // Minimal LVGL ABI shim (no LVGL dep)
│   │   ├── fonts.c / lv_font_stubs.c
│   │   └── // ../../fonts/pixel_font_*.c (generated)
│   ├── storage/
...
`invert_color=true`. SPI link runs at 40 MHz on SPI2_HOST. IDF v5.3 does not
expose the later RGB565 byte-swap flag here, so the renderer stores tile
pixels in big-endian RGB565 before handing them to `esp_lcd`.

**Fonts.** Generated pixel-font variants are 1 bpp with no antialiasing.
The general UI fonts cover ASCII 0x20..0x7E; the IP-address font contains
only `.` and `0`..`9`. The source TTF/OTF files live under top-level `fonts/`
and the C tables are regenerated by `firmware/fonts/generate_pixel_fonts.py`.

| Handle | Source size | line_height | max advance | grid |
|---|---|---|---|---|
| `FONT_DELICATUS_16`  | Delicatus 16 px  | 15 px | 14 px | 1 px |
| `FONT_CAIROPIXEL_32` | Cairopixel 32 px | 20 px | 20 px | 2 px |
| `FONT_QUINQUEFIVE_5` | QuinqueFive 5 px | 5 px  | 6 px  | 1 px |
| `FONT_QUINQUEFIVE_10_DIGITS` | QuinqueFive 10 px | 10 px | 12 px | 2 px |

`FONT_QUINQUEFIVE_10_DIGITS` uses QuinqueFive only for digits. The `.` glyph
is handcrafted as a 2x2 pixel dot with 3 px gaps to neighbouring digits.

Approximate text grid in landscape 160x80:

| Font | Chars / line | Lines / screen |
|---|---|---|
| Delicatus 16  | variable, roughly 11-53 | 5 |
| Cairopixel 32 | variable, roughly 8-40  | 4 |
| QuinqueFive 5 | 26 | 16 |
| QuinqueFive 10 digits | IP up to 15 chars | 8 |
...
LVGL ABI types used by the generated tables (`lv_font_t`, `lv_font_fmt_txt_dsc_t`,
`lv_font_fmt_txt_glyph_dsc_t`, `lv_font_fmt_txt_cmap_t`), so the
auto-generated `.c` files compile as plain C. Two callback-pointer fields
...
1. Looks up the glyph descriptor from the font's FORMAT0_TINY cmap(s).
2. Clips the glyph cell to the visible 160x80 viewport.
3. Allocates a static visible tile in BSS (max 24x24 RGB565 = 1152 B;
   reused across all draws).
4. Pre-fills the tile with `bg`.
5. Walks the 1 bpp glyph box:
   - bit `0` -> keep `bg`
   - bit `1` -> write `fg`
6. Ships the visible tile via `esp_lcd_panel_draw_bitmap()`.
